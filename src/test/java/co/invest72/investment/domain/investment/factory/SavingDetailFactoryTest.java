...
class SavingDetailFactoryTest {
	...
	@DisplayName("단리-적금-년도별 데이터 생성 - 통화가 USD인 경우")
	@Test
	void givenFactory_whenSimpleYearlyUSD_thenReturnDetails() {
		// given
		factory = ((SavingDetailFactory)factory).toBuilder()
			.investmentAmount(new MonthlyInstallmentInvestmentAmount(Money.dollar(1_000_000)))
			.investPeriod(new MonthlyInvestPeriod(13))
		...
		Assertions.assertThat(details.get(0).getInterest()).isEqualTo(Money.dollar(0));
		Assertions.assertThat(details.get(0).getProfit()).isEqualTo(Money.dollar(0));
		Assertions.assertThat(details.get(1).getPrincipal()).isEqualTo(Money.dollar(BigDecimal.valueOf(12_000_000)));
		Assertions.assertThat(details.get(1).getInterest()).isEqualTo(Money.dollar(BigDecimal.valueOf(325_000)));
		Assertions.assertThat(details.get(1).getProfit()).isEqualTo(Money.dollar(BigDecimal.valueOf(12_325_000)));
		Assertions.assertThat(details.get(2).getPrincipal()).isEqualTo(Money.dollar(BigDecimal.valueOf(13_325_000)));
		Assertions.assertThat(details.get(2).getInterest()).isEqualTo(Money.dollar(BigDecimal.valueOf(54_166.67)));
		Assertions.assertThat(details.get(2).getProfit()).isEqualTo(Money.dollar(BigDecimal.valueOf(13_379_166.67)));
	}
