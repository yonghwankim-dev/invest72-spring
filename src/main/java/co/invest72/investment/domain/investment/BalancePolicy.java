...
public enum BalancePolicy implements BalanceStrategy {
	...
	FIXED {
		@Override
		public Money calculate(FinancialProduct product, LocalDate today, LocalDate expirationDate) {
			return Money.of(product.getAmount().getValue(), product.getAmount().getCurrency());
		}
	},
	...
	ACCUMULATIVE {
		...
		public Money calculate(FinancialProduct product, LocalDate today, LocalDate expirationDate) {
			...
			if (today.isAfter(expirationDate)) {
				Money money = Money.of(amount.getValue(), amount.getCurrency());
